...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ad','aciklama','sartlar','tarihKayit'))){
					if($dgr[0]!='@'){
						$araSd.=$ad." LIKE '%".$dgr."%'";
					}
					else{
...
$llimit='';
$yenisayi=0;
$_X=z(1,"WHERE ".$arsvsd.$elyafsorgu.$tipSd.$araSd." ORDER BY sira ASC ".$llimit,NULL,$tbl);
...
if(!empty($_X)){
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			$elyaf=(!empty($_Nesne[$x['nesne_IDelyafTipi']]['metin1'])?$_Nesne[$x['nesne_IDelyafTipi']]['metin1']:'&nbsp;');
			$doviz=(!empty($_Nesne[$x['nesne_IDdoviz']]['metin1'])?$_Nesne[$x['nesne_IDdoviz']]['metin1']:'&nbsp;');
			$uretimTipi=(!empty($_Nesne[$x['nesne_IDuretimTipi']]['metin1'])?$_Nesne[$x['nesne_IDuretimTipi']]['metin1']:'&nbsp;');
			$boyaKontrol=(!empty($_Nesne[$x['nesne_IDboyaKontrol']]['metin1'])?$_Nesne[$x['nesne_IDboyaKontrol']]['metin1']:'&nbsp;');
			$elyafmetin="";
			if(!empty($x['elyaf'])){
				$elyafcek=$x['elyaf'];
				$elyafarray=(!empty($x['elyaf'])?json_decode($x['elyaf'],1):'');
				if(!empty($elyafarray)){
			...
			echo '
			<tr class="allcheck tr'.($i%2?1:2).'" style="cursor: grab;" id="'.$x['ID'].'">
			


				<th class="printx"><input name="sec['.$x['ID'].']" value="1" type="checkbox" class="secilebilir"></th>
				<td class="tdi">'.$yenisayi.'</td>
				...
				<td colspan="1" style="text-align:left;">
					<b>'
					.($admn||$ytkDzn?
					'<a href="?s='.$syf.'&a=detay&id='.$x['ID'].'" class="dty pl-4">'.bulunan('ID',$iplikkart).'</a>'
					:  bulunan('ID',$iplikkart)
				).
				($admn||$ytkDzn?
				...
				: ''
					...
					/*($admn||$ytkDzn?
						'<a href="?s='.$syf.'&a=detay&id='.$x['ID'].'" class="dty">'.bulunan('ID',$iplikkart).'</a>'
						: bulunan('ID',$iplikkart) 
					).
					($admn||$ytkDzn?
				...
				<td>'.$doviz.'</td>
			  <td>'.(!empty($x['fire'])?bulunan('fire',z(36,$x['fire'],2)):'0').' </td> 
				';
