...
<?php if(!empty($vt)&&$vt['arsiv']!='-1'){ ?>
	<div class="dtysayfa">
		<div class="tasiyici">
			<div class="tasiyici1">
					<?php if(!empty($vt['nesne_IDiplikkartTipi'])){ ?>
						<div class="dtyborder dtybaslik2">İPLİK NUMARA KARTI</div>
						<div class="dtyaciklama"><?php if($_Nesne[$vt['nesne_IDiplikkartTipi']]['metin1']){ echo $_Nesne[$vt['nesne_IDiplikkartTipi']]['metin1'];} ?></div>
					<?php } ?>
                    <?php if(!empty($vt['elyaf'])){ ?>
                        <?php 
                        $elyafmetin="";
                        $elyafcek=$vt['elyaf'];
                        $elyafarray=(!empty($vt['elyaf'])?json_decode($vt['elyaf'],1):'');
                        if(!empty($elyafarray)){
						...
						<div class="dtyborder dtybaslik2">ELYAF</div>
						<div class="dtyaciklama"><?php echo $elyafmetin; ?></div>
					<?php } ?>
                    <?php if(!empty($vt['nesne_IDuretimTipi'])){ ?>
						<div class="dtyborder dtybaslik2">İPLİK ÜRETİM TEKNİĞİ</div>
						<div class="dtyaciklama"><?php if($_Nesne[$vt['nesne_IDuretimTipi']]['metin1']){ echo $_Nesne[$vt['nesne_IDuretimTipi']]['metin1'];} ?></div>
					<?php } ?>
					...
					<?php } ?>
					<?php if(!empty($vt['nesne_IDboyaKontrol'])){ ?>
						<div class="dtyborder dtybaslik2">BOYA DURUMU</div>
						<div class="dtyaciklama"><?php if($_Nesne[$vt['nesne_IDboyaKontrol']]['metin1']){ echo $_Nesne[$vt['nesne_IDboyaKontrol']]['metin1'];} ?></div>
					<?php } ?>
                    ...
                    <br>
					<a href="?s=<?php echo z(8,'s'); ?>&a=ekle&farkli=<?php echo z(8,'id'); ?>" class="formduzenleme printx" style="background:#f0ff10;">BU FORMU FARKLI DÜZENLE VE KAYDET</a>
