...
$makinesayi2=0;
$maksayi=0;
$makineyetenegiarray=array();
$makinedegerifaynarray=array();
$makinedegeripusarray=array();
...
if(!empty($vt['pus'])){
	$makinedegeripusarray=(!empty($vt['pus'])?json_decode($vt['pus'],1):'');
	$makinesayi=count($makinedegeripusarray);
}

if(!empty($vt['fayn'])){
	$makinedegerifaynarray=(!empty($vt['fayn'])?json_decode($vt['fayn'],1):'');
}

if(!empty($vt['sistemsayi'])){
	$sistemsayiarray=(!empty($vt['sistemsayi'])?json_decode($vt['sistemsayi'],1):'');
}

if(!empty($vt['ignesayi'])){
	$ignesayiarray=(!empty($vt['ignesayi'])?json_decode($vt['ignesayi'],1):'');
}

if(!empty($vt['makineyetenegi'])){
	$makineyetenegiarray=(!empty($vt['makineyetenegi'])?json_decode($vt['makineyetenegi'],1):'');
	$makinesayi2=count($makineyetenegiarray);
}
?>
	...
	<div class="dtysayfa">
		<div class="tasiyici">
			<div class="tasiyici1">
                    ...
                    <?php if(!empty($makinesayi>0)){  ?>
						<div class="dtyborder dtybaslik2">Pus/Fayn</div>
						<div class="dtyaciklama">
						<?php 
						$degersayisi=1;
						foreach ($makineyetenegiarray as $i => $makid) {
							$makpusarray=(!empty($makinedegeripusarray[$makid])?$makinedegeripusarray[$makid]:'');
							$makfaynarray=(!empty($makinedegerifaynarray[$makid])?$makinedegerifaynarray[$makid]:'');
							if(!empty($makpusarray)||!empty($makfaynarray)){
								foreach ($makpusarray as $maksayi2 => $makpus) {
									echo !empty($makpusarray[$maksayi2])?'<b> Pus: </b>'.$makpusarray[$maksayi2].' ':'';
									echo !empty($makfaynarray[$maksayi2])?'<b> Fayn: </b>'.$makfaynarray[$maksayi2].' <br>':'';
								}
							}
							if($makinesayi!=$degersayisi){
								echo '<br>';
							}
							$degersayisi++;
						}  
					...
					<?php if(!empty($makinesayi>0)){  ?>
						<div class="dtyborder dtybaslik2">Sistem/İğne Sayısı</div>
						<div class="dtyaciklama">
						<?php 
						$degersayisi2=1;
						foreach ($makineyetenegiarray as $i => $makid) {
							$makpusarray=(!empty($sistemsayiarray[$makid])?$sistemsayiarray[$makid]:'');
							$makfaynarray=(!empty($ignesayiarray[$makid])?$ignesayiarray[$makid]:'');
							if(!empty($makpusarray)||!empty($makfaynarray)){
								foreach ($makpusarray as $maksayi2 => $makpus) {
									echo !empty($makpusarray[$maksayi2])?'<b> Sistem Sayısı: </b>'.$makpusarray[$maksayi2].' ':'';
									echo !empty($makfaynarray[$maksayi2])?'<b> İğne Sayısı: </b>'.$makfaynarray[$maksayi2].' <br>':'';
								}
							}
							
							if($makinesayi!=$degersayisi2){
								echo '<br>';
							}
							$degersayisi2++;
						}
					...
					<?php if(!empty($makinesayi2>0)){  ?>
						<div class="dtyborder dtybaslik2">Makine İşlevi</div>
						<div class="dtyaciklama">
						<?php
						foreach ($makineyetenegiarray as $i2 => $makid2) {
							echo !empty($makineyetenegiarray[$makid2])?'<b> Makine İşlevi : </b>'.$nesnemakineyetenegi[$makineyetenegiarray[$makid2]]['metin1'].' <br>':'';
							$maksayi++;
						}  
						?>
						</div>
