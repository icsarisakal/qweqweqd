...
if(!empty($_X)){
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			$ignesayiarray=array();
			if(!empty($x['pus'])){
				$makinedegeripusarray=(!empty($x['pus'])?json_decode($x['pus'],1):'');
				$makinesayi=count($makinedegeripusarray);
			}

			if(!empty($x['fayn'])){
				$makinedegerifaynarray=(!empty($x['fayn'])?json_decode($x['fayn'],1):'');
			}

			if(!empty($x['sistemsayi'])){
				$sistemsayiarray=(!empty($x['sistemsayi'])?json_decode($x['sistemsayi'],1):'');
			}

			if(!empty($x['ignesayi'])){
				$ignesayiarray=(!empty($x['ignesayi'])?json_decode($x['ignesayi'],1):'');
			}

			if(!empty($x['makineyetenegi'])){
				$makineyetenegiarray=(!empty($x['makineyetenegi'])?json_decode($x['makineyetenegi'],1):'');
