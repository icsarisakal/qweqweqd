...
$_X=z(1,"WHERE ".$arsvsd.$iplikkartsorgu.$revizesorgu.$raporsorgusu.$tipSd.$araSd." ORDER BY SUBSTRING(kodu,5) ",NULL,$tbl);
...
if(!empty($_X)){
	...
	if(!empty($_X)){
		foreach ($_X as $key => $veri) {
			if(!empty($veri['kodu'])){
				$veribilgisi=$veri['kodu'].'*_-_*'.$veri["ID"];
				$veribilgisi=str_replace("KIF", "KZF", $veribilgisi);
				$veriarray[]=$veribilgisi;
			}
		}
	}
...
?>
<?php
	$iplikkartlarinitopla='';
	if(empty($phpExcelA)){
		$ysayi=0;
		$ysayi2=0;
		$llimit=str_replace('LIMIT ', '', $llimit);
		foreach($veriarray2 as $key3=>$veri3){
			$x=$_X[$key3];
			if(!empty($llimit)&&$llimit==$ysayi2){
				break;
			}
			$ysayi2++;
			...
			if(!empty($x['iplikkartlari'])){
				...
				if(!empty($iplikkartlariarray)){
					foreach($iplikkartlariarray as $i => $elyf){
						$iplikokuma=!empty(z(1,$i,'','iplik'))?z(1,$i,'','iplik'):'';	
						/*Sonradan eklendi*/$nesne_IDiplikKartTipi = isset($iplikokuma['nesne_IDiplikkartTipi'])?$iplikokuma['nesne_IDiplikkartTipi']:'';
						/*Sonradan eklendi*/$nesne_IDuretimTipi = isset($iplikokuma['nesne_IDuretimTipi'])?$iplikokuma['nesne_IDuretimTipi']:'';
						/*Sonradan eklendi*/$nesne_IDboyaKontrol = isset($iplikokuma['nesne_IDboyaKontrol'])?$iplikokuma['nesne_IDboyaKontrol']:'';
						$iplikkarti=(!empty($_Nesne[$nesne_IDiplikKartTipi]['metin1'])?$_Nesne[$nesne_IDiplikKartTipi]['metin1']:'');
						$uretimTipi=(!empty($_Nesne[$nesne_IDuretimTipi]['metin1'])?$_Nesne[$nesne_IDuretimTipi]['metin1']:'&nbsp;');
    					$boyaKontrol=(!empty($_Nesne[$nesne_IDboyaKontrol]['metin1'])?$_Nesne[$nesne_IDboyaKontrol]['metin1']:'&nbsp;');
						$elyafTipi=(!empty($_Nesne[$i]['metin1'])?$_Nesne[$i]['metin1']:'&nbsp;');
						$iplikkartlarimetin=(!empty($elyf)?'%'.$elyf:''); 
						$elyafmetin="";
						$iplikkartlarinitopla.=$iplikkarti.' , ';
			...
			<tr class="allcheck tr'.($ysayi%2?1:2).'" >
				...
				<td colspan="1" style="text-align:left;">
					<b>'
					.($admn||$ytkDzn?
					'<a href="?s='.$syf.'&a=detay&id='.$x['ID'].'" class="dty pl-4">'.bulunan('ID',$x['kodu']).'</a>'
					: bulunan('ID',$x['kodu']) 
				).
				($admn||$ytkDzn?
				'&nbsp;<a href="?s='.$tbl.'&a='.$kumastipiduzenle.'&id='.$x['ID'].'" ><i class="icon-pencil7 mr-3 icon-1x"></i></a>'
				: ''
				).
					...
					'
			 <td>'.(!empty($resim)?'<a href="#resim" class="icongaleri" onclick="tiklagelsin('.$x['ID'].')" id="myBtn">&nbsp;</a>':'').'</td>
			 ...
			 <td>'.(!empty($iplikkartlarinitopla)?$iplikkartlarinitopla:'&nbsp;').'</td>
			...
			/*if($admn||ytk($tbl,'arsivle')){
				if(empty($x['arsiv']))
					echo '<td class="printx"><a href="?s='.$tbl.'&a=listele&ida1='.$x['ID'].'" title="Siparişi arşive gönder." onClick="return confirm(\'Siparişi arşive göndermek istediğinizden emin misiniz?\');">Arşivle</a></td>';
				...
				<div class="list-icons">
					<div class="dropdown">
						...
						<div class="dropdown-menu dropdown-menu-right">
							...
							<?php } ?>
							<?php if($admn||$ytkDzn){ ?>
								<a href="?s=<?php echo $tbl; ?>&a=<?php echo $kumastipiduzenle; ?>&id=<?php echo $x['ID']; ?>" class="dropdown-item"><i class="icon-file-text2"></i>Düzenle</a>
							<?php } ?>
