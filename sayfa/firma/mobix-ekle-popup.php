...
<div class="popup popup-firma-ekle" style="background-color: #263239;">
    <div class="content-block-login">
      <h4>Yeni Kurum Oluşturun</h4>
      <div class="contactform">
        <form action="ajaxayar.php?s=firma&a=mobix-ekle-popup&ajaxform=1" method="post" id="form-firma-ekle" onsubmit="return ajaxPostGonder( 'form-firma-ekle', function(sonucText){

            if(sonucText && sonucText.length>1){
              var sonucJson=JSON.parse(sonucText);
              if(sonucJson.sonuc==1){
                _FirmaMusteri.push(sonucJson.cevap);
                $('input#musteritakip-ekle-musteritakip-firma_ad').val( sonucJson.cevap.ad );
                firmaAdIleKisilerSelect(sonucJson.cevap.ad);
                myApp.closeModal('.popup-firma-ekle');
              }
              else{
