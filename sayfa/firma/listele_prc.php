...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ad','unvani','tel','fax','eposta','vergiD','vergiNo','	adresFatura','adresSevk'))){
					if($dgr[0]!='@'){
						$araSd.=$ad." LIKE '%".$dgr."%'";
					}
					else{
...
if(!empty($_X)){
	...
	//$_Kisi=z(37,z(1,"WHERE arsiv<>'-1' AND ".z(38,$_X,'kisi_ID'),'ID,ad','kisi'));
		...
		foreach($_X as $i=>$x){$i++;
			...
			<tr class="tr'.($i%2?1:2).'" >
				...
				<td colspan="1">
					...
					($admn||$ytkDzn?
						'<a href="?s='.$syf.'&a=duzenle&id='.$x['ID'].'" class="dty">'.bulunan('ad',$x['ad']).'</a>'
						: bulunan('ad',$x['ad']) 
					).
					($admn||$ytkDzn?
						'<a href="?s=siparistakip&a=ekle&firmaid='.$x['ID'].'" class="ekleicon">Sipariş Ekle</a>'
						: '' 
					).
					/*/
					($admn||$ytkDty?
				...
				<td>'.
					(!empty($_NesneTip[$x['firmaTip']]['metin1'])?$_NesneTip[$x['firmaTip']]['metin1']:'&nbsp;').
				'</td>
				<td>'.(!empty($x['tel'])?bulunan('tel',z(36,$x['tel'],2)):'0').'</td>
				<td>'.(!empty($x['eposta'])?z('metin',bulunan('eposta',$x['eposta'])):'&nbsp;').'</td>
				<td>'.(!empty($x['fax'])?bulunan('fax',z(36,$x['fax'],2)):'0').'</td>
				<td>'.(!empty($x['vergiD'])?z('metin',bulunan('vergiD',$x['vergiD'])):'&nbsp;').'</td>
				<td>'.(!empty($x['vergiNo'])?z('metin',bulunan('vergiNo',$x['vergiNo'])):'&nbsp;').'</td>
				<td>'.(!empty($x['adresFatura'])?z('metin',bulunan('adresFatura',$x['adresFatura'])):'&nbsp;').'</td>
				<td>'.(!empty($x['adresSevk'])?z('metin',bulunan('adresSevk',$x['adresSevk'])):'&nbsp;').'</td>
