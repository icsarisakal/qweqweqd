...
<?php /*/ ?>
<?php $teklif=z(1,array('arsiv'=>0,'ORDER'=>'BY ID DESC'),'','teklif'); ?>
<?php $firmamodul=z(8,'firmaid'); $fm=0; ?>
<?php
$musterid=z(8,'musid'); 
...
<tr>
    ...
    <td>
        <select name="<?php echo $tbl; ?>[teklifno]" class="select2">
            <option value="0" selected>&nbsp;</option>
            <?php foreach ($teklif as $tek) { ?>
                <option value="<?php echo $tek['ID']; ?>" <?php if(!empty($_X['teklifno'])&&$_X['teklifno']==$tek['ID']){ echo 'selected'; } ?>><?php 
                    echo $tek['proformaNo'].(!empty($tek['teklif_ID'])&&!empty($tek['revizeNo'])?'(REV'.$tek['revizeNo'].')':''); 
                ?></option>
            <?php } ?>
        </select>
    </td>
</tr>
<?php if(!empty($firmamodul)){ $fm=$firmamodul; } ?>
<tr>
    <th>Firma/Kurum</th>
    <td>
        <?php echo select(Array('name'=>$tbl.'[firma_ID]','detay'=>'','t'=>'firma','alan'=>'ID,ad','ayrac'=>' /// ','sd'=>"WHERE firmaTip = 0",'icerik'=>'<option value="">Seçiniz</option><option value="yeni">**YENİ**</option>','sec'=>!empty($_X['firma_ID'])?$_X['firma_ID']:$fm)); ?>
    </td>
</tr>
<?php if(z(8,'a')=='ekle'){ ?>
...
<?php if(z(8,'a')=='duzenle'){ ?>
    <?php if(!empty($_OZNSN))foreach($_OZNSN as $ad=>$n){?>
        <?php $_NesneTip=z(37,z(1,"WHERE ad='asamasip'",'ID,ad,sayi1,metin1,metin2','nesne'),'sayi1'); ?>
        <tr>
            <th><?php echo $n['ad']?></th>
            <td class="asamatext">
                <span style="background: black; color: white; padding: 2px;"><?php if(!empty($_NesneTip[$_X['asamasip']]['metin1'])){ echo $_NesneTip[$_X['asamasip']]['metin1']; } ?></span>
...
<tr><th>Sipariş Tarihi</th><td><?php slctrh($tbl.'[tarihSiparis]',!empty($_X['tarihSiparis'])?$_X['tarihSiparis']:z('datetime'))?></td></tr>
<?php /*/ ?><tr><th>Termin Tarihi</th><td><?php slctrh($tbl.'[tarihTermin]',!empty($_X['tarihTermin'])?$_X['tarihTermin']:'')?></td></tr><?php /*/ ?>
<?php if(z(8,'a')=='duzenle'){ ?>
    <?php 
