...
if(z(7,$tbl)){
	...
	$SID=z(2,$_X,$tbl);
	if(!empty($SID)){
		if(!empty($_X['asamasip'])){
			z(2,array('durum'=>$_X['asamasip'],'personel_ID'=>$_X['personel_ID'],'modul_ID'=>$SID,'modul'=>$tbl,'tarih'=>$_X['tarih'],'aciklama'=>$_X['aciklama']),'asama');
		}
		if(!empty($musid)){
			z(3,$musid,array('siparisVar'=>1),'musteritakip');
			z(3,$SID,array('musteritakip_ID'=>$musid),'siparistakip');
		}
		z(33,'ekle',1);
