...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('aciklama','sartlar','tarihKayit','kodu','ismi','sampleName','personel_IDkayit','ad','personel_ID'))){
					if($dgr[0]!='@'){
						$araSd.=$ad." LIKE '%".$dgr."%'";
					}
					else{
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			<th class="printx"><input name="sec['.$x['ID'].']" value="1" type="checkbox" class="secilebilir"></th>
				...
				<td>'.(!empty($cariData[$x['cari_ID']])?$cariData[$x['cari_ID']]['ad']:'&nbsp;').' </td>
				<td>'.(!empty($personelData[$x['personel_IDkayit']])?$personelData[$x['personel_IDkayit']]['ad']:'&nbsp;').' </td>
