...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ad','kdvOran','miktar','fiyat','sartName','aciklama'))){
					if($dgr[0]!='@'){
						$araSd.=$ad." LIKE '%".$dgr."%'";
					}
					else{
...
if(!empty($_X)){
	...
	$_Nesne=z(37,z(1,"WHERE ad='departman' OR ad='ddepartman' OR ad='odemeTipi' OR ad='hizmetTipi'OR ad='miktarBirim' OR ad='tedarikci'",'ID,ad,metin1,metin2','nesne'));
	$_NesneDurum=z(37,z(1,"WHERE ad='gelirGiderDurum'",'ID,ad,sayi1,metin1,metin2','nesne'),'sayi1');
	$_NesneTip=z(37,z(1,"WHERE ad='gelirGiderTip'",'ID,ad,sayi1,metin1,metin2','nesne'),'sayi1');
	$_NesneKdvAnahtar=z(37,z(1,"WHERE ad='kdvAnahtar'",'ID,ad,sayi1,metin1,metin2','nesne'),'sayi1');
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			$_Toplam['gelir'] += !empty($x['gelir'])?$x['gelir']:0;
			...
			echo '
		      <tr class="tr'.($i%2?1:2).' durum_'.$x['durum'].'" >
				...
				'</td>
				<td>'.
					(!empty($_Nesne[$x['nesne_IDodemeTipi']]['metin1'])?$_Nesne[$x['nesne_IDodemeTipi']]['metin1']:'&nbsp;').
				'</td>
				<td>'.
				    (!empty($_Nesne[$x['nesne_IDhizmetTipi']]['metin1'])?$_Nesne[$x['nesne_IDhizmetTipi']]['metin1']:'&nbsp;').
				'</td>
				...
				'</td>
				<td>'.
					(!empty($_NesneKdvAnahtar[$x['kdvAnahtar']]['metin1'])?$_NesneKdvAnahtar[$x['kdvAnahtar']]['metin1']:'&nbsp;').
				'</td>
				<td>'.(!empty($x['kdvOran'])?bulunan('kdvOran',z(36,$x['kdvOran'],2)):'0').'</td>
				<td>'.(!empty($x['miktar'])?bulunan('miktar',z(36,$x['miktar'],2)):'0').'</td>
				  <td>'.
					(!empty($_Nesne[$x['nesne_IDmiktarBirim']]['metin1'])?$_Nesne[$x['nesne_IDmiktarBirim']]['metin1']:'&nbsp;').
				'</td>
				<td>'.(!empty($x['fiyat'])?bulunan('fiyat',z(36,$x['fiyat'],2)):'0').'</td>

				<td>'.(!empty($x['miktar'])&& !empty($x['fiyat'])? ($x['miktar']*$x['fiyat']) :'0').'</td>

				<td>'.(!empty($x['sartName'])?z('metin',bulunan('sartName',$x['sartName'])):'&nbsp;').'</td>
				<td>'.(!empty($x['aciklama'])?z('metin',bulunan('aciklama',$x['aciklama'])):'&nbsp;').'</td>
				 <td>'.
					(!empty($_Nesne[$x['nesne_IDtedarikci']]['metin1'])?$_Nesne[$x['nesne_IDtedarikci']]['metin1']:'&nbsp;').
				'</td>
				<td>'.(!empty($x['tarih'])?z('tarih',$x['tarih']):'&nbsp;').'</td>
				<td>'.(!empty($x['tarihKayit'])?z('tarih',$x['tarihKayit']):'&nbsp;').'</td>
				<td>'.(!empty($x['tarihVade'])?z('tarih',$x['tarihVade']):'&nbsp;').'</td>
				<td>'.(!empty($x['tarihSon'])?z('tarih',$x['tarihSon']):'&nbsp;').'</td>
		...
		<tr id="toplamTutarTr" class="toptr">
			...
			<th>&nbsp;</th>
			<td colspan="9"><b>Toplam Satır Adeti: '.($i).'</b></td>
			<td>&nbsp;</td>
			<td class="toptd" colspan="6"><b>Toplam: '.(!empty($_Toplam['gelir'])?z(36,$_Toplam['gelir'],2,1):'0').' gelir</b></td>
			<td colspan="3">&nbsp;</td>';
