...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ad','aciklama','sartlar','tarihKayit','kodu','ismi','teklifkodu','referans','teklifkonusu'))){
					if($dgr[0]!='@'){
						$araSd.=$ad." LIKE '%".$dgr."%'";
					}
					else{
...
if(!empty($_X)){
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			$cari_id=(!empty($x['cari_ID'])?$x['cari_ID']:0);
			$cari=z(1,$cari_id,'','cari');
			$caribilgi=(!empty($cari['ad'])?$cari['ad']:'').' '.(!empty($cari['soyad'])?$cari['soyad']:'');
			...
			<th class="printx"><input name="sec['.$x['ID'].']" value="1" type="checkbox" class="secilebilir"></th>
				...
				<b>'
				.($admn||$ytkDzn?
				'<a href="?s='.$syf.'&a=detay&id='.$x['ID'].'" class="dty pl-4">'.bulunan('teklifkodu',$x['teklifkodu']).'</a>'
						: bulunan('teklifkodu',$x['teklifkodu'])
				).
				($admn||$ytkDzn?
				...
				<td>'.$caribilgi.'</td>
				<td>'.(!empty($x['teklifkonusu'])?bulunan('teklifkonusu',$x['teklifkonusu']):'&nbsp;').'</td>
				<td>'.(!empty($x['referans'])?bulunan('referans',$x['referans']):'&nbsp;').'</td>
				';
				?>
