...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ad','aciklama','sartlar'))){
					if($dgr[0]!='@'){
						$araSd.=$ad." LIKE '%".$dgr."%'";
					}
					else{
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			<tr class="allcheck tr'.($i%2?1:2).'" >
				...
				<td colspan="1">
					...
					($admn||$ytkDzn?
						'&nbsp;<a href="?s='.$tbl.'&a=duzenle&id='.$x['ID'].'" class="icon-pencil7 mr-3 icon-1x"></a>'
						: ''
					).
					'
				</td>
				...
				<td>'.(!empty($x['renkKodu'])?z('metin',bulunan('renkKodu',$x['renkKodu'])):'&nbsp;').' </td> 
				<td>'.(!empty($x['barkodNo'])?z('metin',bulunan('barkodNo',$x['barkodNo'])):'&nbsp;').' </td> 
				<td>'.(!empty($x['artikelNo'])?z('metin',bulunan('artikelNo',$x['artikelNo'])):'&nbsp;').' </td> 
		...
		echo ' 
			<tr id="toplamTutarTr" class="toptr">
				...
				<th>&nbsp;</th>
				<td colspan="7">&nbsp;</td>
				<td class="toptd" colspan="1"><b>Toplam: '.(!empty($_Toplam['adet'])?z(36,$_Toplam['adet'],0):'0').' adet</b></td>
