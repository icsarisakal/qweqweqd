...
<?php if(!empty($vt)&&$vt['arsiv']!='-1'){ ?>
	<div class="dtysayfa">
		<div class="tasiyici">
			<div class="tasiyici1">
					<?php if(!empty($vt['ad'])){  ?>
						<div class="dtyborder dtybaslik2">Kumaş Türü Adı</div>
						<div class="dtyaciklama"><?php if(!empty($vt['ad'])){ echo $vt['ad']; } ?></div>
					<?php } ?>

					<?php if(!empty($vt['makinacesitleri_ID'])){ 
						$makinacesitleri=z(1,$vt['makinacesitleri_ID'],'','makinacesitleri');
						$makinedegeripusarray=(!empty($makinacesitleri['pus'])?json_decode($makinacesitleri['pus'],1):'');
						$makinedegerifaynarray=(!empty($makinacesitleri['fayn'])?json_decode($makinacesitleri['fayn'],1):'');
						$makinesayi=count($makinedegeripusarray);
						$makinedegeriarray=(!empty($makinacesitleri['makineyetenegi'])?json_decode($makinacesitleri['makineyetenegi'],1):'');
						$makineislevi=(!empty($vt['nesne_IDmakineYetenegi'])?$vt['nesne_IDmakineYetenegi']:'');
						...
						if(!empty($makinedonguid)||$makinedonguid==0){
							...
							foreach ($makinedongupus as $mkdpus => $mkdongu) {
								$makinedegerleri2[$mkdpus]=(!empty($mkdongu)?'Pus: '.$mkdongu:'');
								$makinedegerleri2[$mkdpus].=(!empty($makinedongufayn[$mkdpus])?' Fayn: '.$makinedongufayn[$mkdpus]:'');
							}
						}
						/*
						$pusmakine='';
						if(!empty($vt['pusvefayn'])){
							$pusmakine=$makinedegeripusarray[$vt['pusvefayn']][0];
						}
						$faynmakine='';
						if(!empty($vt['pusvefayn'])){
							$faynmakine=$makinedegerifaynarray[$vt['pusvefayn']][0];
						}
						*/
						$pusvefaymmetin='';
						$pusvefaynarray=(!empty($vt['pusvefayn'])?json_decode($vt['pusvefayn'],1):'');
						if(!empty($pusvefaynarray)){
							foreach($pusvefaynarray as $pfarray){
								$pusvefaymmetin.=(!empty($makinedegeripusarray[$pfarray][0])?'<b>Pus: </b>'.$makinedegeripusarray[$pfarray][0].'&nbsp;':'');
								$pusvefaymmetin.=(!empty($makinedegerifaynarray[$pfarray][0])?'<b>Fayn: </b>'.$makinedegerifaynarray[$pfarray][0].'<br>':'');
							}
						}
						...
						if(!empty($kumaspusvefayn)){
							foreach($kumaspusvefayn as $pf => $pfarray){
								...
								$pusvefaynmetni2=' Pus: '.$makinedongupus[$pfarray];
								$pusvefaynmetni2.=' Fayn: '.$makinedongufayn[$pfarray];
							   
								$pusvefaynjson[$pf]=$pusvefaynmetni2;
							}
						}
						 ?>
						<div class="dtyborder dtybaslik2">Makina Adı</div>
						<div class="dtyaciklama"><?php if(!empty($makinacesitleri['ad'])){ echo $makinacesitleri['ad'].' '; } ?></div>
					<?php } ?>

					<?php if(!empty($pusvefaynjson)){ ?>
						<div class="dtyborder dtybaslik2">Makina Özellikleri</div>
						<div class="dtyaciklama"><b>
						<?php
						if(!empty($pusvefaynjson)){
							$makinedegerlerisayi=count($pusvefaynjson);
							foreach ($pusvefaynjson as $mkdd => $mkdegerleri2) {
								echo $mkdegerleri2.' ';
								if($makinedegerlerisayi!=$mkdd){
									echo '<br>';
								}
							}
						}
					...
					<?php if(!empty($vt['fasonmaliyet'])){  ?>
						<div class="dtyborder dtybaslik2">Fason Örgü Maliyeti</div>
						<div class="dtyaciklama"><?php if(!empty($vt['fasonmaliyet'])){ echo $vt['fasonmaliyet']; } ?></div>
					<?php } ?>
