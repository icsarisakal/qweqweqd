...
if(!empty($_X)){
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			$_Makinacesitleri=z(1,$x['makinacesitleri_ID'],'','makinacesitleri');
			$makinedegeripusarray=(!empty($_Makinacesitleri['pus'])?json_decode($_Makinacesitleri['pus'],1):'');
			$makinedegerifaynarray=(!empty($_Makinacesitleri['fayn'])?json_decode($_Makinacesitleri['fayn'],1):'');
			$makinesayi=count($makinedegeripusarray);
			$makinedegeriarray=(!empty($_Makinacesitleri['makineyetenegi'])?json_decode($_Makinacesitleri['makineyetenegi'],1):'');
			$makineislevi=(!empty($x['nesne_IDmakineYetenegi'])?$x['nesne_IDmakineYetenegi']:'');
			$kumaspusvefayn=(!empty($x['pusvefayn'])?$x['pusvefayn']:'');
			$makinedonguid='';
			$pusvefaynjson='';
			...
			if(!empty($kumaspusvefayn)){
				foreach($kumaspusvefayn as $pf => $pfarray){
					...
					$pusvefaynmetni2=(!empty($makinedongupus[$pfarray])?' Pus: '.$makinedongupus[$pfarray]:'');
					$pusvefaynmetni2.=(!empty($makinedongufayn[$pfarray])?' Fayn: '.$makinedongufayn[$pfarray]:'');
			...
			/*
			$pusmakine='';
			if(!empty($x['pusvefayn'])){
				$pusmakine=$makinedegeripusarray[$x['pusvefayn']][0];
				if(!empty($makinedegeripusarray[$x['pusvefayn']][0])){
					$pusmakine='<b>Pus: </b>'.$pusmakine.'&nbsp;';
				}
				
			}
			$faynmakine='';
			if(!empty($x['pusvefayn'])){
				$faynmakine=$makinedegerifaynarray[$x['pusvefayn']][0];
				if(!empty($makinedegerifaynarray[$x['pusvefayn']][0])){
				$faynmakine='<b>Fayn: </b>'.$faynmakine.'<br>';
				}
			}
			*/
			...
			<td>'.
				(!empty($_Makinacesitleri['ad'])?$_Makinacesitleri['ad']:'&nbsp;').
			'</td>
			...
			<td>'.
				(!empty($x['fasonmaliyet'])?z(36,$x['fasonmaliyet']):'0').
			'</td>
			<td>'.$doviz.'</td>
