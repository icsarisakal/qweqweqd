...
if(!empty($_X)){
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			echo '
				<td>'.(0&&($admn||ytk($tbl,'duzenle'))?'<a href="?s='.$tbl.'&a=duzenle&id='.$x['ID'].'" class="dty">'.z('tarihsaat',$x['tarih']).'</a>':z('tarihsaat',$x['tarih'])).'</td>
				<td>'.(!empty($x['USD'])?blnn('usd',z("36",$x['USD'],2)):'&nbsp;').'</td>
				<td>'.(!empty($x['EUR'])?blnn('eur',z("36",$x['EUR'],2)):'&nbsp;').'</td>
				<td>'.(!empty($x['GBP'])?blnn('gbp',z("36",$x['GBP'],2)):'&nbsp;').'</td>
				';
