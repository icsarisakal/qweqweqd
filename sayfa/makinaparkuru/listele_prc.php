	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			<th class="printx"><input name="sec['.$x['ID'].']" value="1" type="checkbox" class="secilebilir"></th>
				...
				<td>'.(!empty($x['plakasayi'])&&$x['plakasayi']=='1'?'Tek':'&nbsp;').(!empty($x['plakasayi'])&&$x['plakasayi']=='2'?'Çift':'&nbsp;').' </td>

				<td>'.(!empty($x['pus'])?$x['pus']:'&nbsp;').' </td>

				<td>'.(!empty($x['fayn'])?$x['fayn']:'&nbsp;').' </td>

				<td>'.(!empty($x['sistemsayi'])?$x['sistemsayi']:'&nbsp;').' </td>

				<td>'.(!empty($x['ignesayi'])?$x['ignesayi']:'&nbsp;').' </td>
