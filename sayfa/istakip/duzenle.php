...
if(z(8,'id')){
	...
	//if(!z(5,"WHERE arsiv='0' AND ad='".$_X['ad']."' AND ID<>'".$ID."'")){
		...
		if(z(3,$ID,$_X)){
			$_Log['sonuc']=1;
			$_Log['mesaj']="[PERSONEL] isimli kullanıcı ".$ID." nolu iş takibini(musteritakip) düzenledi.";
			require('parca/log.php');
			if(!empty($pa)){
				z(2,array('durum'=>$pa,'personel_ID'=>$personel,'modul_ID'=>$ID,'modul'=>$tbl,'tarih'=>$tarih,'aciklama'=>$_X['aciklama']),'asama');
			}
			z(33,'duzenle',1);
			z('git','yenile');
		}
			...
			$_Log['sonuc']=0;
			$_Log['mesaj']="[PERSONEL] isimli kullanıcı ".$ID." nolu iş takibini(musteritakip) düzenleyeme çalıştı fakat başarısız oldu.";
			require('parca/log.php');
			z(33,'duzenle','-1');
		}
	//}
