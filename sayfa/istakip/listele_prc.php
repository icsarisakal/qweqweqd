...
if(!empty($changeasama)&&$changeasamaid){
	z(3,$changeasamaid,array('asamais'=>$changeasama),$tbl);
...
if(z(8,'a')=='listele_durum0'){
	$durumsorgu='AND asamais = 0';
}

if(z(8,'a')=='listele_durum1'){
	$durumsorgu='AND asamais = 1';
}

if(z(8,'a')=='listele_durum2'){
	$durumsorgu='AND asamais = 2';
}

if(z(8,'a')=='listele_durum3'){
	$durumsorgu='AND asamais = 3';
}

if(z(8,'a')=='listele_durum4'){
	$durumsorgu='AND asamais = 4';
}

if(z(8,'a')=='listele_durum5'){
	$durumsorgu='AND asamais = 5';
}

if(z(8,'a')=='listele_durum6'){
	$durumsorgu='AND asamais = 6';
}

if(z(8,'a')=='listele_durum7'){
	$durumsorgu='AND asamais = 7';
}

if(z(8,'a')=='listele_durum8'){
	$durumsorgu='AND asamais = 8';
}

if(z(8,'a')=='listele_durum9'){
	$durumsorgu='AND asamais = 9';
}

if(z(8,'a')=='listele_durum10'){
	$durumsorgu='AND asamais = 10';
}

if(z(8,'a')=='listele_durum11'){
	$durumsorgu='AND asamais = 11';
}

if(z(8,'a')=='listele_durum12'){
	$durumsorgu='AND asamais = 12';
}

if(z(8,'a')=='listele_durum13'){
	$durumsorgu='AND asamais = 13';
}

if(z(8,'a')=='listele_durum14'){
	$durumsorgu='AND asamais = 14';
}

if(z(8,'a')=='listele_durum15'){
	$durumsorgu='AND asamais = 15';
}

// yetki ozel1 : Herkesin Girdilerini Görebilme
$ytkOzel1=ytk('istakip','ozel1');
$sahipSd=(!$admn&&!$ytkOzel1?" AND personel_ID='".$bnmID."'":'');
$_X=z(1,"WHERE ".$arsvsd.$sahipSd.$durumsorgu.$araSd." ORDER BY ID ".(empty($phpExcelA)?'DESC':'ASC').$llimit,NULL,$tbl);
if(!empty($_X)){
	...
	$_NesneTip=z(37,z(1,"WHERE ad='asamais'",'ID,ad,sayi1,metin1,metin2,metin3','nesne'),'sayi1');
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			echo '
			<tr class="tr'.($i%2?1:2).' asama_'.$x['asamais'].' "  >
			...
			<form action="" method="post">
				<td>
					<input type="hidden" class="asamaid" name="asamaid" value="<?php echo $x['ID']; ?>">
					<select name="asama" class="asamaguncelle">
						<option value="0" selected>&nbsp;</option>
						<?php foreach ($_NesneTip as $ntip) {?>
							<option value="<?php echo $ntip['sayi1']; ?>"<?php if(!empty($_NesneTip[$x['asamais']]['metin1'])&&$_NesneTip[$x['asamais']]['sayi1']==$ntip['sayi1']){ echo 'selected'; } ?>><?php echo $ntip['metin1']; ?></option>
						<?php } ?>
					</select>
				</td>
			</form>
		...
		foreach($_X as $i=>$x){$i++;
			...
			->setCellValue('D'.($i+1),(!empty($_NesneTip[$x['asamais']]['metin1'])?$_NesneTip[$x['asamais']]['metin1']:'-'))
