...
<?php if(!empty($vt)&&$vt['arsiv']!='-1'){ ?>
				...
				<table cellpadding="0" cellspacing="0">
					<tbody>
						...
						<?php if(!empty($vt['asamais'])){ ?>
							<tr><th>Durum</th><td><?php if($_NesneDurum[$vt['asamais']]['metin1']){ echo $_NesneDurum[$vt['asamais']]['metin1'];} ?></td></tr>
						<?php } else {  ?>
							<tr><th>Durum</th><td>Henüz Okunmadı</td></tr>
						<?php } ?>
						<?php if(!empty($vt['tarihKayit'])){ ?>
							<tr><th>Kayıt Tarihi</th><td><?php _z('tarih',$vt['tarihKayit']); ?></td></tr>
						<?php } else {  ?>
	...
	<?php 
	$hatirlatici=z(1,array('arsiv'=>0,'hmodul'=>'istakip','hmodul_ID'=>$id),'','hatirlatici');
	...
	<?php if(!empty($asamacek)){ ?>
		<div class="sayfa" style="background: #eeeeee; overflow: hidden;">
			...
			<?php foreach ($asamacek as $ascek) { ?>
				<style type="text/css">
					<?php 
					$_NesneAsama=z(37,z(1,"WHERE ad='asamais'",'ID,ad,sayi1,metin1,metin2','nesne'),'sayi1');
