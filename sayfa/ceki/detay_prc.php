	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			echo '<tr '.($i%2==0?'class="tr2"':'').'>
				<td>'.$i.'</td>';
			echo '
				<td>'.( $kodx ?( '<a href="'.( $kodx ?('?s=stok&a=detay&id='.$x['ID']):('?stok='.z(34,$x['ID'])) ).'" class="dty">'.blnn('ID',$x['ID']).'</a>' ) :$x['ID']).'</td>
				<td>'.(!empty($_Hamkumasstok[$x['hamkumasstok_ID']]['numuneIsmi'])?$_Hamkumasstok[$x['hamkumasstok_ID']]['numuneIsmi']:'&nbsp;').'</td>
				<td>'.(!empty($x['dokumastok_ID'])?blnn('dokumastok_ID',$x['dokumastok_ID']):'&nbsp;').'</td>
				<td>'.(!empty($_Hamkumasstok[$x['hamkumasstok_ID']]['kumasIsmi'])?$_Hamkumasstok[$x['hamkumasstok_ID']]['kumasIsmi']:'&nbsp;').'</td>
				...
				<td>'.(!empty($x['enHam'])?blnn('enHam',z(36,$x['enHam'],2)):'&nbsp;').' / '.(!empty($x['hkHamGramaj'])?blnn('hkHamGramaj',z(36,$x['hkHamGramaj'],2)):'&nbsp;').'</td>
