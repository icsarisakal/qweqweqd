	...
	foreach($_G as $ad=>$dgr){
		if(isset($dgr)){
			if($dgr!==''){
				...
				if(is_array($dgr)&&count($dgr)==2&&in_array($ad,Array('tarih','tarihIslem'))){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ID','aciklama','numuneIsmi','tarih','dokSalTopNo','notlar'))){
					$araSd.=$ad." LIKE '%".$dgr."%'";
				}
				else if(in_array($ad,Array('top1K','top1A','top2K','mamulMetraj','cekmeBoy','cekmeEn'))){
...
$hambezSd=z(8,'hambezstok_ID','sayi')?" AND hambezstok_ID='".z(8,'hambezstok_ID','sayi')."' AND giris='hambezstok'":"";
...
//echo "WHERE ".$arsvsd.$araSd.$doksipSd." ORDER BY ID ".(empty($phpExcelA)?'DESC':'ASC').$llimit;
$_X=z(1,"WHERE tip='0' AND ".$arsvsd.$araSd.$doksipSd.$bytSd.$bytsipSd.$hambezSd.$firmaSd.$girisSd." ORDER BY ID ".(empty($phpExcelA)?'DESC':'ASC').$llimit,NULL,$tbl);
if(!empty($_X)){
	...
	$_Nesne=z(37,z(1,("WHERE ".z(38,$_X,'nesne_IDkalite')." OR ".z(38,$_X,'nesne_IDboyahane')." OR ".z(38,$_X,'nesne_IDdokumaSalonu')." OR ad='boyahane'"),'ID,metin1,metin2','nesne'));
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			switch ($x['cikis']) {
				...
				case 'mamulbezsatis':
					$x['cikisAdi']=!empty($_Firma[$x['firma_IDiade']]['ad'])?($_Firma[$x['firma_IDiade']]['ad']):'';
					break;
				...
				case 'dokumastok':
					$x['cikisAdi']=!empty($_Nesne[$x['nesne_IDdokumaSalonu']]['metin1'])?($_Nesne[$x['nesne_IDdokumaSalonu']]['metin1']):'';
					break;
			...
			switch ($x['giris']) {
				...
				case 'mamulbezsatis':
					$x['girisAdi']=!empty($_Firma[$x['firma_ID']]['ad'])?($_Firma[$x['firma_ID']]['ad']):'';
					break;
				...
				case 'boyatakip':
					$x['girisAdi']=!empty($_Nesne[$x['nesne_IDboyahane']]['metin1'])?($_Nesne[$x['nesne_IDboyahane']]['metin1']):'';
					break;
			...
			echo '
				<td><a href="?s='.$syf.'&a=detay&id='.$x['ID'].'" class="dty">'.blnn('ID',$x['ID']).'</a></td>
				'./*/'
