...
<div class="sayfa">
	...
	'					<tr>
							...
							/></td>
							<td>'.select(Array(
								'name'=>'stok[nesne_IDkalite][]','t'=>'nesne','alan'=>'ID,metin1','sd'=>"WHERE ad='kalite' ORDER BY ID ASC",'detay'=>'class="buyuk-select" id="araNesne_IDkalite" tabindex="2"',
						            	//'icerik'=>'<option value="">-Seçiniz-</option>',
	...
	'					<tr>
	    			...
	    			<thead class="bunun-icini-kopyala" style="display: none" id="bunu-kopyala-id">
						<?php kopyaTrSablon(); ?>
	    			</thead>
	    			<?php } ?>
		    ...
		    <input type="hidden" name="cikisboyatakip_ID" value="<?php 
				$_Cekicikisfrm=z(7,'cekicikisfrm');
				echo !empty($_Cekicikisfrm['boyatakip_ID'])?$_Cekicikisfrm['boyatakip_ID']:'';
		     ?>" />
		     <input type="hidden" name="boyahaneCikisPartiNo" value="<?php 
				$_Cekicikisfrm=z(7,'cekicikisfrm');
				echo !empty($_Cekicikisfrm['boyahaneCikisPartiNo'])?$_Cekicikisfrm['boyahaneCikisPartiNo']:'';
		     ?>" />
		     <input type="hidden" name="cikisfirma_ID" value="<?php 
				$_Cekicikisfrm=z(7,'cekicikisfrm');
				echo !empty($_Cekicikisfrm['cikisfirma_ID'])?$_Cekicikisfrm['cikisfirma_ID']:'';
		     ?>" />
...
<?php
	if(empty($barkodluA)){
		require(__DIR__.'/mamulsevkbarkod_js.php');
	}
	else{
