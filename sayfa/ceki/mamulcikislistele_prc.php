...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ID','aciklama','numuneIsmi','tarih','boyahanePartiNo'))){
					$araSd.=$ad." LIKE '%".$dgr."%'";
				}
...
$_X=z(1,"WHERE tip='1' AND 
		giris<>'mamulbezstokdetay' AND 
		giris<>'numunebezstokdetay' AND 
		".$arsvsd.$araSd.$doksipSd.$bytSd.$bytsipSd.$mmlBzStkDtySd.$firmaSd.$girisSd." ORDER BY ID ".(empty($phpExcelA)?'DESC':'ASC').$llimit,NULL,$tbl);
if(!empty($_X)){
	...
	$_Nesne=z(37,z(1,("WHERE ".z(38,$_X,'nesne_IDkalite')." OR ".z(38,$_X,'nesne_IDboyahane')." OR ".z(38,$_Mamulkumas,'nesne_IDbOzellik')),'ID,metin1,metin2','nesne'));
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			switch ($x['cikis']) {
				...
				case 'mamulbezsatis':
					$x['cikisAdi']=!empty($_Firma[$x['firma_ID']]['ad'])?($_Firma[$x['firma_ID']]['ad']):'FİRMA';
					break;
			}
			...
			echo '
				<td><a href="?s='.$syf.'&a=mamuldetay&id='.$x['ID'].'" class="dty">'.blnn('ID',$x['ID']).'</a></td>
				'./*/'
