...
$_Nesne=z(37,z(1,"WHERE ad='' OR ad='doviz' OR ad='birimTipi' OR ad='boyaBaskiHizmetleri'",'ID,ad,metin1,metin2','nesne'));
	...
	if(empty($phpExcelA)){
		...
		foreach($_X as $i=>$x){$i++;
			...
			<tr class="allcheck tr'.($i%2?1:2).'" >
			  ...
			  <td>'.(!empty($depo)?$depo:'').' </td> 
			  <td>'.(!empty($x['malzemeKodu'])?$x['malzemeKodu']:'').' </td>
			  <td>'.(!empty($x['malzemeAd'])?$x['malzemeAd']:'').' </td>
			  <td>'.(!empty($x['aciklama'])?$x['aciklama']:'').' </td>
			  <td>'.(!empty($x['siparisMiktar'])?$x['siparisMiktar']:'').' </td>
			  <td>'.(!empty($birim)?$birim:'').' </td>
			  <td>'.(!empty($x['fiyat'])?$x['fiyat']:'').' </td>
			  <td>'.(!empty($doviz)?$doviz:'').' </td>
			  <td>'.(!empty($x['gelenMiktar'])?$x['gelenMiktar']:'').' </td>
			  <td>'.(!empty($x['kalanMiktar'])?$x['kalanMiktar']:'').' </td>
			  <td>'.(!empty($x['tarihTermin'])?$x['tarihTermin']:'').' </td>
			  <td>'.(!empty($x['tarihFis'])?$x['tarihFis']:'').' </td>
