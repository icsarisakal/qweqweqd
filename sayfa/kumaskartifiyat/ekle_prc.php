...
setInterval(function() {
        $.ajax({
            type:"POST",
            url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu5",
...
function fiyatguncelle(ths) {
    $.ajax({
            type:"POST",
            url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu5&kapat=1",
            success:function(gelensorgu){
                var boyabaskiall=gelensorgu.cevap.boyabaskiall;
                var nesnedoviz=gelensorgu.cevap.nesnedoviz;
                if(boyabaskiall!=null&&boyabaskiall!=''){
    ...
    $(document).ready(function(){
        ...
        $.ajax({
            type:"POST",
            url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu2",
            success:function(gelensorgu){
                var boyabaskiall=gelensorgu.cevap.boyabaskiall;
                var nesnedoviz=gelensorgu.cevap.nesnedoviz;
                window.boyabaskiall=boyabaskiall;
        ...
        $('#makinacesidi').change(function() {
            ...
            $.ajax({
                type:"POST",
                url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&kumas="+makinacesidi_ID,
                success:function(gelensorgu){
                    if(gelensorgu.sonuc==1){
                        ...
                        if(pusvefaynjson!=null){
                            ...
                            $.each(pusvefaynjson, function(k, v) {
                                ...
                                clonemuz+='<td><label for="fname'+k+'" class="labelimiz">'+v+':</label><input type="checkbox" id="fname'+k+'" name="kumaskartifiyat[pusvefayn]['+k+'][]"></td><td><span style="float: left;margin: 10px;"> EN: </span><input type="text" value="0" class="form-control" style="float: left;width:auto;" name="kumaskartifiyat[pusvefayn]['+k+'][]" autocomplete="off"><span style="float: left;margin: 10px;">cm</span><br></td>';
                                clonemuz+='</tr>';
        ...
        $('#kumasturu').change(function() {
            ...
            $.ajax({
                type:"POST",
                url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&kumasturu="+kumasturu_ID,
                success:function(gelensorgu){
                    if(gelensorgu.sonuc==1){
                        ...
                        if(pusvefaynjson!=null){
                            ...
                            $.each(pusvefaynjson, function(k, v) {
                                ...
                                clonemuz+='<td><label for="fname'+k+'" class="labelimiz">'+v+':</label><input type="checkbox" id="fname'+k+'" name="kumaskartifiyat[pusvefayn]['+k+'][]"></td>';
                                clonemuz+='<td><span style="float: left;margin: 10px;"> EN: </span><input type="text" value="0" name="kumaskartifiyat[pusvefayn]['+k+'][]" class="form-control" style="float: left;width:auto;" autocomplete="off"><span style="float: left;margin: 10px;">cm</span><br></td>';
                                clonemuz+='</tr>';
                                $(".pusvefayntbody").append(clonemuz);
                            });
                        }
...
function resimsil(ths){
    var resimsil=$(ths).parent().find("input").val();
        $.ajax({
            type:"POST",
            url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&resimsil="+resimsil,
            success:function(gelensorgu){
                $(ths).parent().remove();
...
$('.iplikkartihepsi tr').each(function(i, obj2){
    ...
    var eachbirimfiyatinput=$(obj2).find(".iplikbirimdoviz input").val();
    if(eachbirimfiyatinput){
        eachbirimfiyat=eachbirimfiyatinput;
    }
...
function boyamaliyetiyansit(ths){
    ...
    $.ajax({
        type:"POST",
        url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&boyabaski="+boyabaski_ID+"&fasonorgumaliyet="+fasonorgumaliyet+"&cekilenhamkumastl="+cekilensonhamkumastl+"&mkurusd="+manuelkurusd+"&mkureur="+manuelkureur,
        success:function(gelensorgu){
            var boyabaskiformul=gelensorgu.cevap.boyabaskiformul;
            //inputlar=inputlar+'<input type="text" name="kumaskartifiyat[boyamaliyet]['+boyabaskitipi+']['+boyabaski_ID+'][]" value="" autocomplete="off">';
...
function boyamaliyetiyansityeni(ths){
    ...
    var cekilenhamtl2=sy(cekilenhamtl);
    var fasonorgumaliyet=cekilenhamtl;
    var birimfiyatimanuel=$(ths).parent().find("input").val();
    birimfiyatimanuel=parseFloat(birimfiyatimanuel);
    var cekilensonhamkumastl=$('.ekhizmetler').find("tbody:last-child .tlekboyamaliyeti input").val();
    ...
    var manueldengelenfiyat=$(ths).parent().find(".boyafiyatmanuel").val();
    manueldengelenfiyat=parseFloat(manueldengelenfiyat);
    var manueldengelenfire=$(ths).parent().find(".boyafiremanuel").val();
    manueldengelenfire=parseFloat(manueldengelenfire);
    ...
    if(manueldengelenfire){
        y_ustboyabaskifire=manueldengelenfire;
        y_ustboyabaskifirekontrol=manueldengelenfire;
        y_ustboyabaskifirekontrol=sy(y_ustboyabaskifirekontrol,2);
    }
    ...
    if(manueldengelenfiyat){
        y_ustboyabaskifiyat=manueldengelenfiyat;
    }
    ...
    y_ustboyabaskifiyat=y_ustboyabaski["fiyat"];
...
function boyamaliyetiyansityeni2(ths){
    ...
    if(birimfiyatimanuel){
        y_ustboyabaskifiyat=birimfiyatimanuel;
    }
...
function boyabaskitetikle(ths){
    var selectboyabaski=$(ths).parent().find(".selectboyabaski");
    boyamaliyetiyansityeni2(selectboyabaski);
}
...
function siraylacalistir2(){
    ...
    $.ajax({
        type:"POST",
        url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu",
        success:function(gelensorgu){
...
function altboyadegistir(ths){
    ...
    $('.selectboyatd select').each(function(i, obj3){
        ...
        $.ajax({
            type:"POST",
            url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&ustboyabaski="+iplik_ID+"&altboyabaski="+boyabaski_ID+"&fasonorgumaliyet="+fasonorgumaliyet+"&cekilenhamkumastl="+cekilensonhamkumastl+"&cekilenfasonorgu"+cekilenhamfasonorgumaliyeti+"&mkurusd="+manuelkurusd+"&mkureur="+manuelkureur,
            success:function(gelensorgu){
                if(window.selectboyatdanahtar==0){
                    siradakisayac++;
                    window.selectboyatdanahtar=1;
                ...
                var input='<input type="text" name="kumaskartifiyat[boyamaliyet]['+iplik_ID+']['+boyabaski_ID+']" value="'+ustboyabaski+'" autocomplete="off" class="boyamaliyethesapla boyainput'+boyainput+'" disabled><input type="hidden" name="kumaskartifiyat[boyamaliyet]['+iplik_ID+']['+boyabaski_ID+']" value="'+ustboyabaski+'" autocomplete="off" class="boyamaliyethesapla">';
                ...
                if(alttabloindex==3){
                    ...
                    var input='<input type="text" name="kumaskartifiyat[boyamaliyet]['+iplik_ID+']['+boyabaski_ID+']" value="'+toplameskiveyenifiyat+'" autocomplete="off" class="boyamaliyethesapla boyainput'+boyainput+'" disabled><input type="hidden" name="kumaskartifiyat[boyamaliyet]['+iplik_ID+']['+boyabaski_ID+']" value="'+toplameskiveyenifiyat+'" autocomplete="off" class="boyamaliyethesapla">';
...
function dovizver(sorgu){
    ...
    var toplaminput3=fiyathesaplausd;
    /*
    var toplaminput='<input type="text" class="form-control" value="'+toplaminput+'" autocomplete="off"disabled><input type="hidden" name="kumaskartifiyat[fiyatlar][]" value="'+toplaminput+'">';
    var toplaminput2='<input type="text" class="form-control" value="'+toplaminput2+'" autocomplete="off"disabled>';
    var toplaminput3='<input type="text"  class="form-control"value="'+toplaminput3+'" autocomplete="off"disabled>';
...
function altboyadegistiryeni(ths){
    var boyabaski_ID = $(ths).parent().parent().find(".selectmaliyet select option:selected").val();
    if(boyabaski_ID=='0'){
        boyabaski_ID='10';
    }
    var boyabaskiust_ID = $(ths).parent().parent().find(".selectmaliyet select").attr("class");
    var boyabaskiustfiyat = $('.'+boyabaskiust_ID).attr("id");
    var boyamaliyetlerimizlength=$('.tablealt .boyamaliyetlerimiz').length;
    ...
    var alttabloindex=$(ths).parent().parent().find(".selectmaliyet select").parent().parent().index();
    ...
    $('.selectboyatd select').each(function(i, obj4){
        ...
        var sondegisenfiyat=$(ths).parent().parent().find(".sondegisenfiyat").val();
        sondegisenfiyat=parseFloat(sondegisenfiyat);

        var sondegisenfire=$(ths).parent().parent().find(".sondegisenfire").val();
        sondegisenfire=parseFloat(sondegisenfire);
        ...
        if(sondegisenfire){
            y_altboyabaskifire=sondegisenfire;
            y_altboyabaskifiresade=sondegisenfire;
        }
        ...
        if(sondegisenfiyat){
            y_altboyabaskifiyat=sondegisenfiyat;
        }
        ...
        // console.log("...................");
        // console.log("y_altbaskibirimfiyat");
        // console.log(y_altbaskibirimfiyat);
        // console.log("usttengelenfiyat");
        // console.log(usttengelenfiyat);
        // console.log("yenialtfiyathesaplamatl");
        // console.log(yenialtfiyathesaplamatl);
        // console.log("y_altbaskitoplam");
        // console.log(y_altbaskitoplam);
        // console.log("-------------------");
                ...
                var input='<input type="text" name="kumaskartifiyat[boyamaliyet]['+iplik_ID+']['+boyabaski_ID+']" value="'+altbaskitoplam+'" autocomplete="off" class="boyamaliyethesapla form-control boyainput'+boyainput+'" disabled><input type="hidden" name="kumaskartifiyat[boyamaliyet]['+iplik_ID+']['+boyabaski_ID+']" value="'+altbaskitoplam+'" autocomplete="off" class="boyamaliyethesapla">';
                window.yinput=window.yinput+input;
                ...
                if(karihesapla!=0){
                    ustboyabaskikar=sy(ustboyabaski)*karihesapla;
                    var toplaminput=ustboyabaskikar.toFixed(2);
                    var toplaminput='<input type="text" value="'+toplaminput+'" autocomplete="off" class="form-control" disabled><input type="hidden" name="kumaskartifiyat[fiyatlar][]" value="'+toplaminput+'">';
                ...
                if(alttabloindex==3){
                    ...
                    var toplameskiveyenifiyat1=sy(inputsec+ustboyabaski);
                    var toplameskiveyenifiyat=toplameskiveyenifiyat1.toFixed(2);
                    var input='<input type="text" name="kumaskartifiyat[boyamaliyet]['+iplik_ID+']['+boyabaski_ID+']" value="'+altbaskitoplam2+'" autocomplete="off" class="boyamaliyethesapla form-control boyainput'+boyainput+'" disabled><input type="hidden" name="kumaskartifiyat[boyamaliyet]['+iplik_ID+']['+boyabaski_ID+']" value="'+altbaskitoplam2+'" autocomplete="off" class="boyamaliyethesapla">';
                    window.yinput2=window.yinput2+input;
                    $('.'+iplikclass+' .boyamaliyetiinput1').html(window.yinput2);
                ...
                //altboyadegistir2();
                //altboyahamhizmet();
                y_altboyabaskifiyat=y_altboyabaski["fiyat"];
                y_altboyabaskifiresade=y_altboyabaski["fire"];
...
function boyamaliyetihizmetyansit2(ths){
    ...
    $.ajax({
        type:"POST",
        url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&boyabaskialt="+select_ID+"&fasonorgumaliyet="+fasonorgumaliyet+"&cekilenhamkumastl="+cekilenhamkumastl+"&mkurusd="+manuelkurusd+"&mkureur="+manuelkureur,
        success:function(gelensorgu){
...
function altboyahamhizmet(ths){
    ...
    $('.selectboyatd select').each(function(i, obj){
        ...
        $.ajax({
            type:"POST",
            url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&ustboyabaski="+iplik_ID+"&altboyabaski="+boyabaski_ID+"&fasonorgumaliyet="+fasonorgumaliyet+"&cekilenfasonorgu"+cekilenhamfasonorgumaliyeti,
            success:function(gelensorgu){
...
function boyadegisimi(ths){
    var boya_ID = $(ths).val();
    var namehazirla="kumaskartifiyat[boyamaliyet]["+boya_ID+"][]";
...
?>
<tr><th>Kumaş Kodu</th><td><input type="text" name="<?php echo $tbl?>[kodu]" value="<?php echo !empty($_X['kodu'])?$_X['kodu']:''?>" autocomplete="off" class="1kumaskodu form-control" disabled></td></tr>
<tr><th>Kumaş İsmi</th><td><input type="text" name="<?php echo $tbl?>[ismi]" value="<?php echo !empty($_X['ismi'])?$_X['ismi']:''?>" autocomplete="off" class="1kumasismi form-control" disabled></td></tr>
<input type="hidden" name="<?php echo $tbl?>[nesne_IDkumasTipi]" value="175">
<tr>
    <th>Article</th><td>
    <input type="text" class="form-control" name="<?php echo $tbl?>[article]" value="<?php echo (!empty($_X['article'])?$_X['article']:''); ?>" disabled>
</tr>
<?php
$metinhazirla='';
$iplikkartlarimetin="";
...
<tr>
    <th>Makina Türü</th>
    <td>
    <?php echo select(Array('name'=>$tbl.'[makinacesitleri_ID]','detay'=>' class="select2 select-search" id="makinacesidi" onchange="iplikdegisimkur()" disabled','t'=>'makinacesitleri','alan'=>'ID,ad','ayrac'=>' /// ','sd'=>"",'icerik'=>'<option value="">Seçiniz</option>','sec'=>!empty($_X['makinacesitleri_ID'])?$_X['makinacesitleri_ID']:0)); ?>
    </td>
</tr>
<tr>
    <th>Kumaş Türü</th>
    ...
    ?>
    <select name="<?php echo $tbl; ?>[kumasturu_ID]" id="kumasturu" class="select2 select-search" onchange="iplikdegisimkur()" disabled>
    ...
    if($_X['makinacesitleri_ID']){
        ...
        if(!empty($kumaspusvefayn)){
            foreach($kumaspusvefayn as $pf => $pfarray){
                ...
                $pusvefayndeger=(!empty($pfarray[1])?$pfarray[1]:'');
...
<tr class="pusvefayn pusvefayntr">
    <td>
        ...
        <input type="checkbox" id="fname<?php echo (!empty($pfj)?$pfj:'0'); ?>" name="kumaskartifiyat[pusvefayn][<?php echo (!empty($pfj)?$pfj:'0'); ?>][]" <?php echo (!empty($pfjson['check'])&&$pfjson['check']=='on'?'checked':''); ?>>
    </td>
    <td class="kumaseni">
        <span>EN: </span><input type="text" value="<?php echo (!empty($pfjson['deger'])?$pfjson['deger']:''); ?>" name="kumaskartifiyat[pusvefayn][<?php echo (!empty($pfj)?$pfj:'0'); ?>][]" autocomplete="off" class="form-control"><span> cm</span><br>
    </td>
</tr>
<?php } } ?>
    ...
    <?php if(z(8,'a')!='ekle'&&empty($farkli)){ ?>
        $('#resimfile').on("change", function(){
            ...
            for (i = 0; i < file_length; i++) {
                ...
                $.ajax({
                    url: 'ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu3&id='+id,
                    dataType: 'text',  
                    cache: false,
                    contentType: false,
                    processData: false,
...
function iplikdegisim(ths){
    $(document).ready(function(){
        ...
        var manuelkurcins=$(".mkurdovizcinsi select option:selected").val();
        var manuelkurcins='74';
        $(ths).parent().parent().find("input:nth-child(1)").attr('name', 'kumaskartifiyat[iplikkartlari]['+iplik_ID+']');
        $(ths).parent().parent().find("input:nth-child(3)").attr('name', 'kumaskartifiyat[iplikfireleri]['+iplik_ID+']');
        var consolbilgi="iplik_ID=>"+iplik_ID+"mkurusd=>"+manuelkurusd+"mkureur=>"+manuelkureur+"mkurdovizcinsi=>"+manuelkurcins;
            ...
            if(iplik_ID){
                ...
                $.ajax({
                    type:"POST",
                    url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&iplik="+iplik_ID+"&mkurusd="+manuelkurusd+"&mkureur="+manuelkureur+"&mkurdovizcinsi="+manuelkurcins,
                    success:function(gelensorgu){
...
function iplikdegisimkur(ths){
    $(document).ready(function(){
        ...
        $('.iplikkartihepsi tr').each(function(i, obj7){
            ...
            $.ajax({
                type:"POST",
                url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&iplik="+iplik_ID+"&mkurusd="+manuelkurusd+"&mkureur="+manuelkureur+"&mkurdovizcinsi="+manuelkurcins+"&getiplikoran="+iplikoran,
                success:function(gelensorgu){
...
function karorandegistir(ths){
    $(document).ready(function(){
        var kar_ID = $(ths).val();
        $(ths).parent().parent().find("input:nth-child(1)").attr('name', 'kumaskartifiyat[karoranlari]['+kar_ID+']');
        siraylacalistir();
        /*/
        $.ajax({
            type:"POST",
            url:"ajaxayar.php?s=kumaskartifiyat&a=ajaxsorgu&iplik="+kar_ID+"&mkurusd="+manuelkurusd+"&mkureur="+manuelkureur+"&mkurdovizcinsi="+manuelkurcins,
            success:function(gelensorgu){
...
<tr>
  ...
  <tr id="iplikkartlari1" class="iplikkarti1" style="cursor: grab;">
    ...
    <td class="iplikbirimdoviz"><span class="iplikbirimfiyat"></span> <div>-</div> <input type="text" class="form-control" name="<?php echo $tbl; ?>[iplikbirim][0]" onkeyup="hesapla(this)" value=""> </td>
    <td class="iplikfiresi"><div style="float:left;">0</div> <span style="float:left;display:none;">&nbsp; => &nbsp;</span> <input type="text" onkeyup="hesapla(this)" name="<?php echo $tbl; ?>[iplikfireleri][0]" class="form-control"  autocomplete="off" style=""></td>
  ...
  <?php $toplam=0; ?>
    ...
    <?php 
        $iplikkartlaricek=$_X['iplikkartlari'];
        $iplikkartlariarray=(!empty($_X['iplikkartlari'])?json_decode($_X['iplikkartlari'],1):'');
        $iplikbirimarray=(!empty($_X['iplikbirim'])?json_decode($_X['iplikbirim'],1):'');
        if(!empty($iplikkartlariarray)){
            $iplikkartlariarray=str_replace('puan','',$iplikkartlariarray);
        }
        ...
        $sayi=0;
        if(!empty($iplikkartlariarray)){ foreach($iplikkartlariarray as $i => $iplkkrtlari){ $sayi++; $toplam=$toplam+$iplkkrtlari; $iplkbirim=(!empty($iplikbirimarray[$i])?$iplikbirimarray[$i]:''); ?>
    ...
    <td class="iplikbirimdoviz"><span class="iplikbirimfiyat"></span> <div>-</div> <input type="text" class="form-control" value="<?php echo $iplkbirim; ?>" name="<?php echo $tbl; ?>[iplikbirim][<?php echo $i; ?>]" onkeyup="hesapla(this)"> </td>
    <td class="iplikfiresi"><div style="float:left;">0</div> <span style="float:left;display:none;">&nbsp; => &nbsp;</span> <input type="text" onkeyup="hesapla(this)" name="<?php echo $tbl; ?>[iplikfireleri][<?php echo $i; ?>]" class="form-control" autocomplete="off" value="<?php echo (!empty($iplikfireleriarray[$i])?$iplikfireleriarray[$i]:''); ?>" style=""></td>
    <td class="iplikfiyat">0</td>
    <td class="iplikfire">0</td>
    <td class="iplikfirelifiyat">0</td>
    <td class="iplikfirelifiyattl">0</td>
    ...
    <?php } else { ?>
        <tr id="iplikkartlari1" class="iplikkarti1" style="cursor: grab;">
            ...
            <td class="iplikoran"><input type="text" class="iplikoranlari form-control" onkeyup="hesapla(this)" name="<?php echo $tbl; ?>[iplikkartlari][0]" autocomplete="off" value="0"></td>
            <td class="iplikbirimdoviz"><span class="iplikbirimfiyat"></span> <div>-</div> <input type="text" class="form-control"name="<?php echo $tbl; ?>[iplikbirim][0]" onkeyup="hesapla(this)"> </td>
            <td class="iplikfiresi"><div style="float:left;">0</div> <span style="float:left;display:none;">&nbsp; => &nbsp;</span> <input type="text" onkeyup="hesapla(this)" name="<?php echo $tbl; ?>[iplikfireleri][0]" class="form-control"  autocomplete="off" style=""></td>
            <td class="iplikfiyat">0</td>
            <td class="iplikfire">0</td>
            <td class="iplikfirelifiyat">0</td>
...
<tr style="border-bottom: 4px solid #94989e;" class="boyabaskitr">
    ...
    <div class="bilgikutucugu">Birim Fiyatı Manuel</div>
    <div class="bilgikutucugu">Firesi</div>
    <div class="bilgikutucugu">Firesi Manuel</div>
    ...
    <input type="text" class="form-control boyafiyatmanuel" style="border: 1px solid #000;" autocomplete="off" onkeyup="boyabaskitetikle(this)" name="<?php echo $tbl; ?>[boyabirimfiyatmanuel][]" value="">
    <p style="display:block;border: 1px solid #ddd;background: white;padding: 10px 20px;margin:0;">&nbsp;</p>
    <input type="text" class="form-control boyafiremanuel" style="border: 1px solid #000;" autocomplete="off" onkeyup="boyabaskitetikle(this)" name="<?php echo $tbl; ?>[boyabirimfiremanuel][]" value="">
    ...
    $boyabirimfiyatmanuelarray=(!empty($_X['boyabirimfiyatmanuel'])?json_decode($_X['boyabirimfiyatmanuel'],1):'');
    $boyabirimfiremanuelarray=(!empty($_X['boyabirimfiremanuel'])?json_decode($_X['boyabirimfiremanuel'],1):'');
    $sayi=0;
    if(!empty($boyamaliyetarray)){ foreach($boyamaliyetarray as $bymlyt => $bymlytler){ $sayi++;
        $boyabirimfiyatmanuel=(!empty($boyabirimfiyatmanuelarray[$bymlyt])?$boyabirimfiyatmanuelarray[$bymlyt]:'');
        $boyabirimfiremanuel=(!empty($boyabirimfiremanuelarray[$bymlyt])?$boyabirimfiremanuelarray[$bymlyt]:'');
        ...
        if(!empty($boyabirimfiyatmanuel)){
            $ustboyabaski=$boyabirimfiyatmanuel;
        }
        $ustboyabaskifire=(!empty($ustboyabaskicek['fire'])?$ustboyabaskicek['fire']:0);
        if(!empty($boyabirimfiremanuel)){
            $ustboyabaskifire=$boyabirimfiremanuel;
        }
    ...
    <input type="text" class="form-control boyafiyatmanuel" style="border: 1px solid #000;" autocomplete="off" onkeyup="boyabaskitetikle(this)" name="<?php echo $tbl; ?>[boyabirimfiyatmanuel][<?php echo $bymlyt; ?>]" value="<?php echo $boyabirimfiyatmanuel; ?>">
    ...
    <input type="text" class="form-control boyafiremanuel" style="border: 1px solid #000;" autocomplete="off" onkeyup="boyabaskitetikle(this)" name="<?php echo $tbl; ?>[boyabirimfiremanuel][<?php echo $bymlyt; ?>]" value="<?php echo $boyabirimfiremanuel; ?>">
...
<tr class="mamulboyamaliyeti">
    ...
    <th colspan="1" class="ekth" style="font-size:16px;">BİRİM FİYAT && Manuel Fiyat</th>
    <th colspan="1" class="ekth" style="font-size:16px;">BİRİM FİYAT ₺</th>
    <th colspan="1" class="ekth" style="font-size:16px;">FİRE && Manuel Fire</th>
    <th colspan="1" class="ekth" style="font-size:16px;">SATIR TOPLAMI ₺</th>
    </tr>    
<?php if(z(8,'a')=='ekle'&&empty($farkli)){ ?>
...
<td><input type="text" disabled="disabled" class="form-control yetkikontrol2" value="0"><input type="text" onkeyup="altboyadegistiryeni(this)" class="form-control sondegisenfiyat" name="<?php echo $tbl; ?>[sonbirimfiyatmanuel][]" value=""></td>
<td><input type="text" disabled="disabled" class="form-control yetkifiyattl" value="0"></td>
<td><input type="text" disabled="disabled" class="form-control yetkikontrolfire" value="0"><input type="text" onkeyup="altboyadegistiryeni(this)" class="form-control sondegisenfire" name="<?php echo $tbl; ?>[sonbirimfiremanuel][]" value=""></td>
...
<?php if(!empty($bymlytler)){ ?>
    ...
    $sayi=0;
    $sayi2=0;
    $sonbirimfiyatmanuelarray=(!empty($_X['sonbirimfiyatmanuel'])?json_decode($_X['sonbirimfiyatmanuel'],1):'');
    $sonbirimfiremanuelarray=(!empty($_X['sonbirimfiremanuel'])?json_decode($_X['sonbirimfiremanuel'],1):'');
    ...
    $sonbirimfiyatmanuel=(!empty($sonbirimfiyatmanuelarray[$bymlyt])?$sonbirimfiyatmanuelarray[$bymlyt]:'');
    $sonbirimfiremanuel=(!empty($sonbirimfiremanuelarray[$bymlyt])?$sonbirimfiremanuelarray[$bymlyt]:'');
   ...
   <td><input type="text" disabled="disabled" class="form-control yetkikontrol2" value="<?php echo $altboyabaskisade.$dovizsimge; ?>"><input type="text" onkeyup="altboyadegistiryeni(this)" class="form-control sondegisenfiyat" name="<?php echo $tbl; ?>[sonbirimfiyatmanuel][<?php echo $bymlyt; ?>]" value="<?php echo $sonbirimfiyatmanuel; ?>"></td>
    <td><input type="text" disabled="disabled" class="form-control yetkifiyattl" value="<?php echo $altboyabaski; ?>"></td>
    <td><input type="text" disabled="disabled" class="form-control yetkikontrolfire" value="<?php echo $altboyabaskifiresade; ?>"><input type="text" onkeyup="altboyadegistiryeni(this)" class="form-control sondegisenfire" name="<?php echo $tbl; ?>[sonbirimfiremanuel][<?php echo $bymlyt; ?>]" value="<?php echo $sonbirimfiremanuel; ?>"></td>
   ...
   <?php 
    ...
    ?>
   <input type="text" name="kumaskartifiyat[boyamaliyet]<?php echo $bymlyt2; ?>][<?php echo $bymlyt; ?>]" value="<?php echo $bymlytler2[$bymlyt]; ?>" autocomplete="off" class="boyamaliyethesapla boyainput<?php echo $sayi; ?> form-control" style="width:<?php echo $boyamaliyetstyle; ?>;" disabled="">
   <input type="hidden" name="kumaskartifiyat[boyamaliyet][<?php echo $bymlyt2; ?>][<?php echo $bymlyt; ?>]" value="<?php echo $bymlytler2[$bymlyt]; ?>" autocomplete="off" class="boyamaliyethesapla form-control">
   <?php } } ?>
   </td>
   </tr>
    <?php } } else { ?>
