...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ad','tipi','aciklama','sartlar','renkKodu','varyant','ozelKodu','erisimKodu','mamulKodu','pantoneNo'))){
					if($dgr[0]!='@'){
						$araSd.=$ad." LIKE '%".$dgr."%'";
					}
					else{
...
if(!empty($_X)){
	...
	$_Nesne=z(37,z(1,"WHERE ad='doviz' OR ad='birimTipi'",'ID,ad,metin1,metin2','nesne'));
 	$boyaHane=z(37,z(1,"WHERE arsiv='0'",'','cari'));
 	$boyabaski=z(37,z(1,"WHERE arsiv='0'",'','boyabaski')); 
 	$musteri=z(37,z(1,"WHERE arsiv='0' AND nesne_IDcariTipi='178'",'ID,ad,unvan','cari')); 
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			<tr class="allcheck tr'.($i%2?1:2).'" >
			    ...
			    <td>'.($x['boyabaski_ID']!=0?$boyabaski[$x['boyabaski_ID']]['tipi']:'').'</td>
				<td>
				<a href="#" id="myBtn" onClick=tabloGoster('.$x['ID'].');><i class="icon-table"></i></a>
...
<tr>
	...
	<th>Renk Adı</th>
				...
				<td>'.(!empty($x['varyant'])?z('metin',bulunan('varyant',$x['varyant'])):'').' </td> 
				<td>'.(!empty($x['fiyat'])?bulunan('fiyat',z(36,$x['fiyat'],2)):'0').' </td> 
				<td>'.($x['nesne_IDbirimTipi']!='0'&&$_Nesne[$x['nesne_IDbirimTipi']]['ad']=='birimTipi'?$_Nesne[$x['nesne_IDbirimTipi']]['metin1']:'').' </td> 
				<td>'.($x['nesne_IDdoviz']!='0'&&$_Nesne[$x['nesne_IDdoviz']]['ad']=='doviz'?$_Nesne[$x['nesne_IDdoviz']]['metin1']:'').' </td> 
...
<script>
function tabloGoster(i){
var table = '';
if($(".modal").css('display')=='block'){
	$(".tablegroup tr:eq(1)").nextAll().remove();
	$(".modal").css('display','none');
}

     $.ajax({
           type:"POST",
           url:"ajaxayar.php?s=renkkarti&a=ajaxsorgu2&id="+i,
           success:function(gelensorgu){
                var dataTablo=gelensorgu.cevap.dataTablo;
				table= '';
				$(".ayniclass").remove();

              for (var j = 0 ; j < dataTablo.length ; j++) {
				  if(dataTablo[j]['ad']!=0){
				  table += '<tr class="ayniclass"><td>'+dataTablo[j]['ad']+'</td><td>'+dataTablo[j]['boyabaski']+'</td><td>'+dataTablo[j]['metin1']+'</td></tr>';			  
				  
				  }
			}
			$(".tablegroup tr:eq(1)").after(table);
