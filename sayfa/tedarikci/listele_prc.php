...
if(!empty($_G)){
	...
	foreach($_G as $ad=>$dgr){
		...
		if($ad=='durum'){print_r($dgr);die;}
				...
				else if(in_array($ad,Array('ad','soyad','telCep1','telCep2','telSabit','telFax','eposta','adres','aciklama'))){
					if($dgr[0]!='@'){
						$araSd.=$ad." LIKE '%".$dgr."%'";
					}
					else{
...
if(!empty($_X)){
	...
	$_Nesne=z(37,z(1,"WHERE ad='tedarikciTipi'",'ID,ad,metin1,metin2','nesne'));
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			<tr class="allcheck tr'.($i%2?1:2).' durum_'.$x['durum'].'" >
				...
				<td>'.(!empty($x['soyad'])?bulunan('soyad',$x['soyad']):'&nbsp;').'</td>
				<td>'.
				...
				<td>'.(!empty($x['telSabit'])?bulunan('telSabit',$x['telSabit']):'&nbsp;').'</td>
