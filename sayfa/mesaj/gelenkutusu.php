...
-->
          ...
          <div class="yenimesaj" ><a id="modalac" href="#" data-toggle="modal" data-target="#myModal" class="text-default"><img class="yeniMesajIcon" src="upload/kumaskarti/yeniMesaj.png" alt=""></a></div>
        ...
        <!--mesajlaşma alanı -->
        <div class="mesgs">
          <div class="msg_history" onscroll="eskiOku();">
...
// baslangic scroll yukarı kaydırınca 5 tane daha mesaj okur
function eskiOku(){
 
   
  if($('.msg_history').scrollTop()==0){

     var mesajX= $('.msg_history').children().find('div').first().attr('class');
      
    var idM= $('.msg_history').children().find('div').first().attr('id');
    var idP= $('.msg_history').attr('id');
    var ic='5';
    var personelID=<?php echo z('lgn','ID'); ?>

    console.log('İDM. '+idM+ 'İDP: '+idP+' personel: '+personelID);
      
      $.ajax({
        
        url:'ajaxayar.php?s=mesaj&a=ajaxsorgu&durum=eskioku&idm='+idM+'&idp='+idP+'&ic='+ic,
        success:function(e){
          
          var eskiData=e.cevap.eskioku;
          console.log(eskiData);
         if(eskiData!=null){
                eskiData.forEach(function(item,index){
                  
                    if(item.personel_IDgonderen!=personelID){

                      $('.msg_history > div').prepend('<div id="'+item.ID+'" class="incoming_msg"><div class="incoming_msg_img"> <img src="https://ptetutorials.com/images/user-profile.png" alt="sunil"> </div><div class="received_msg"><div class="received_withd_msg"><p>'+item.mesaj+'</p><span class="time_date">'+item.tarih+'</span></div></div></div>');

                    }
                    else {
                      
                      $('.msg_history > div').prepend('<div id="'+item.ID+'" class="outgoing_msg"><div class="sent_msg"><p>'+item.mesaj+'</p><span class="time_date">'+item.tarih+'</span> </div></div>');
...
// bitis scroll yukarı kaydırınca 5 tane daha mesaj okur

 let dMyenile=null;
let gelenKisiler=[];
let geciciarray=[];
let kisisayisi=[];
 function ekle(gelenyenimesaj,el,id,personel_IDgonderen,ad,tarih,mesaj,sayisi,img){
  ...
  if(sayisi<=0){
            ...
            el.prepend('<div onclick="mesajoku(this)" value="'+ad+'" id="'+id+'" name="'+personel_IDgonderen+'" class="chat_list" id="chat" ><div class="chat_people"   ><div class="chat_img"> <img src="upload/kumaskarti/'+img+'" alt="sunil"> </div><div class="chat_ib"><h5 >'+ad+'<span class="chat_date">'+tarih+'</span></h5><p>'+mesaj+'</p></div></div><div class="numarator"></div</div>');
          ...
          gelenyenimesaj.prepend('<div onclick="mesajoku(this)" value="'+ad+'" id="'+id+'" name="'+personel_IDgonderen+'" class="chat_list" style="background-color:#FFEAC9;" id="chat" ><div class="chat_people"   ><div class="chat_img"> <img src="upload/kumaskarti/'+img+'" alt="sunil"> </div><div class="chat_ib"><h5 >'+ad+'<span class="chat_date">'+tarih+'</span></h5><p>'+mesaj+'</p></div></div><div class="numarator "><span class="badge badge-pill bg-warning-400 ml-auto ml-md-0">'+sayisi+'</span></div></div>');
        ...
        success:function(e){
         ...
         // console.log('gelen kutusu yenile');
          ...
          data.forEach(function(key,value){
              ...
              ekle(gelenyenimesaj,el,key.ID,personel_IDgonderenYeni,key.ad,key.tarih,key.mesaj,key.yeniMesaj,imgYeni);
