...
if(!empty($_X)){
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			$gonderen=z(1,'WHERE ID='.$x['personel_IDgonderen'].'','ad','personel');
			$alici=z(1,'WHERE ID='.$x['personel_IDalici'].'','ad','personel');
			...
			<th class="printx"><input name="sec['.$x['ID'].']" value="1" type="checkbox" class="secilebilir"></th>
				...
				<td>'.(!empty($gonderen[0])?$gonderen[0]:'&nbsp;').' </td>
				<td>'.(!empty($alici[0])?$alici[0]:'&nbsp;').' </td>
				<td>'.(!empty($x['mesaj'])?$x['mesaj']:'&nbsp;').' </td>
