	...
	foreach($_G as $ad=>$dgr){
		if(isset($dgr)){
			if($dgr!==''){
				...
				if(is_array($dgr)&&count($dgr)==2&&in_array($ad,Array('tarih','tarihKayit','tarihTakvim'))){
		...
		if($ad=='durum'){print_r($dgr);die;}
					...
					foreach($dgr as $i=>$dg){
						if($dg!==''){
							if(!empty($fea))$araSd.=' OR ';else $fea++;
							$dg=str_replace(array('_bos','_sifir'),array('','0'),$dg);
							$araSd.=$ad.(in_array($ad,Array('tarih','tarihKayit','tarihTakvim'))?" LIKE '%".$dg."%'":"='".$dg."'");
						}
					}
					$araSd.=')';
				}
...
if(!empty($_X)){
	...
	if(empty($phpExcelA)){
		foreach($_X as $i=>$x){$i++;
			...
			$phpExcelCols=array('A1'=>'NO','B1'=>'İLGİLİ FİRMA','C1'=>'SON DURUM','D1'=>'AÇIKLAMA','E1'=>'TARİH'
		//	,'F1'=>'ÜNVANI','G1'=>'TEL','H1'=>'FAX','I1'=>'E-POSTA','J1'=>'VERGİ DAİRESİ','K1'=>'VERGİ NO','L1'=>'FATURA ADRESİ','M1'=>'SEVK ADRESİ'
		);
			$basH='A'; $sonH='E';
			...
			foreach($_X as $i=>$x){$i++;
				...
				->setCellValue('C'.($i+1),(!empty($_NesneTip[$x['asama']]['metin1'])?$_NesneTip[$x['asama']]['metin1']:''))
				->setCellValue('D'.($i+1),(!empty($x['aciklama'])?$x['aciklama']:''))
				->setCellValue('E'.($i+1),z('tarihsaat',$x['tarihKayit']))
										  /*/
